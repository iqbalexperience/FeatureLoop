...
interface CommentSectionProps {
  ...
  isClosed: boolean; // feedback is closed
}

export function CommentSection({ feedbackId, initialComments, isAuthorized, isClosed }: CommentSectionProps) {
            ...
            <div className="flex-1 space-y-1">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {(isAdmin || comment.user.id === session?.user?.id) && !isOptimistic && !comment.isDeleted && (
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-8 px-2 text-destructive hover:text-destructive"
                      disabled={isAdmin ? false : isClosed}
