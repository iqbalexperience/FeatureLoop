      ...
      <CardHeader className="pb-2 flex flex-row justify-between items-start">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            <Badge variant={getStatusVariant(task.status)} className="uppercase">
        ...
        <div className="flex gap-2">
          {canCreateChangelog && (
            <Button variant="secondary" size="sm" asChild>
              <Link href={`/changelog/new?task=${task.id}`}>
          ...
          {canDelete && (
            <Link href={`/board/${task.id}/delete`} className={buttonVariants({ variant: "destructive", size: "sm" })}>
        ...
        {task.changelog && (
          <div className="mb-4 p-3 bg-accent/10 rounded-md">
            <p className="text-sm font-medium flex items-center">
              ...
              <Link
                href={`/changelog/${task.changelog.id}`}
